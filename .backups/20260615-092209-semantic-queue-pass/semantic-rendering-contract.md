...
### Quote, gallery, post terms, video, and audio contract
...
The following surfaces are the active next-pass expansion targets for fuller schematization and are intended to complete the core and BuddyPress portions of the semantic layer.

These surfaces MUST be added to this contract when their runtime behavior changes:
...
- `core/search`
- `core/tag-cloud`
- BuddyPress dynamic listing blocks including `bp/dynamic-members` and `bp/dynamic-groups`
...
These targets are not fully governed beyond their currently documented runtime behavior until code for that next pass exists.
